...
In this step, we will find one 10xVisium sample from `spatialLIBD` package, indexed by "Br8667_mid" by subsetting from the overall dorsolateral pre-frontal cortex (DLPFC) dataset `spatialDLPFC_Visium`. 
...
We also apply an optional step, creating an categorical variable named `spDomain`.  
```{r data.subset}
...
Here we first apply the `add_fill` to add the spots color-coded by the total umi counts of all spots(`sum_umi`).  
